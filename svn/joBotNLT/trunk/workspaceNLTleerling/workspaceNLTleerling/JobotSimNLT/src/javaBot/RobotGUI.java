...
//			System.out.println("LED=" + i + " " + ledStatus[i].getForeground());
	...
	/** Create the LCD display textfields
	 * 
	 * @return JPanel - the panel with input and output textfields
	 */
	private JPanel createLCD(JPanel mainPanel )
	{
		if (robot.getLCD() != null) {
			JPanel cp = new JPanel();
			cp.setBounds(5, 280, 170, 50);
			cp.setBackground(Color.LIGHT_GRAY);
			cp.setBorder(new TitledBorder(null, "  LCD Display", TitledBorder.LEADING, TitledBorder.TOP, new Font("Dialog", Font.PLAIN, 10), Color.BLACK));
			cp.setLayout(null);
			lcdDisplay = new JTextArea();
//			lcdDisplay.setText("LINE 001\n" + "LINE 002\n");
			lcdDisplay.setBackground(Color.LIGHT_GRAY);
			lcdDisplay.setBounds(20, 15, 80, 30);
			lcdDisplay.setEditable(false);
			lcdDisplay.setForeground(Color.BLACK);
			lcdDisplay.setFont(new Font("Dialog", Font.BOLD, 12));
			cp.add(lcdDisplay);
			mainPanel.add( cp );
			return cp;
		}
	...
	private void createSensors( JPanel mainPanel)
	{
		JPanel cp = new JPanel();
        if (robot.getLCD() != null)
        	cp.setBounds(5, 330, 170, (robot.getSensors().length * 50));
        else
...
//		 Initialize actor status labels
	...
	private void updateLCD() {
		if (robot.getLCD() != null) {
			lcdDisplay.setText(robot.getLCD().lcdText + "\n" +
				           robot.getLCD().lcdText1);
		}
	}
