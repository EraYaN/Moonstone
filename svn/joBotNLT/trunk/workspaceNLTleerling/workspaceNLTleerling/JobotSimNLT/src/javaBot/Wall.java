 ...
 * 					   1.2 Corrected drawing error - 26/7/2008
...
public class Wall extends NonMovableObject implements IDragable {
	 ...
	 * Image is 512x512 pixels. Wall is 194 pixels wide, borders are 194 pixels	 */
	public Wall() {
		super("Wall");
		diameter = 0.1;
	...
	public void setLocation(Location OrgLocation) {
		...
		Line[] lWall;
		lWall = new Line[4];
		lWall[0] = new Line(new Location(wallX, wallY), new Location(wallX
				+ wallWidth, wallY));
		lWall[1] = new Line(new Location(wallX, wallY), new Location(wallX,
				wallY + wallHeight));
		lWall[2] = new Line(new Location(wallX, wallY + wallHeight),
				new Location(wallX + wallWidth, wallY + wallHeight));
		lWall[3] = new Line(new Location(wallX + wallWidth, wallY),
				new Location(wallX + wallWidth, wallY + wallHeight));
	...
	public double getWidth() {
		width = this.getGraphicalRepresentation().getWidth()
				/ Simulator.pixelsPerMeter;
		width *= 0.22; // Width is 194/512 pixels
		return width;
	}
