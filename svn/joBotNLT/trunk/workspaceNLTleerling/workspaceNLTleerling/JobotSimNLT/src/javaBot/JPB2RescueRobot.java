 ...
 *         the JPB2Robot
 */
public class JPB2RescueRobot extends UVMRobot
{
	private static final String	IMAGE_FILE			= Simulator
															.getRelativePath("./resources/jobot.gif");
	private static final String	CONFIG_XML			= Simulator.getRelativePath("./JoBot/Rescue/jobot.xml");
	 ...
	 * @param positionY
	 */
	public JPB2RescueRobot(String name, double positionX, double positionY)
	{
		/*
		 * friction = 7.0
		 * diameter = 2.0
		...
		try
		{
			...
			getSensors()[3] = addSensor("IRSensor", BASE_RADIUS, 60, 270);					// Left
			getSensors()[4] = addSensor("IRSensor", BASE_RADIUS, 120, 270);					// Right
			getSensors()[5] = addSensor("FieldSensor", BASE_RADIUS, 300);					// Left / Middle
			getSensors()[6] = addSensor("FieldSensor", BASE_RADIUS, 240);					// Left / Middle
		}
