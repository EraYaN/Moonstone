...
public class UVMDemo extends UVMRunnable implements AgentWebService,
	...
	public static final int STATE_LINE_FOLLOWER = 4;
	public static final int STATE_WALL_FOLLOWER = 8;
	public static final int STATE_MAX_DIP 		= 9;
	private String DIP = "0=Idle ,1=Calib,2=Flee ,3=     ,4=Line ,5=     ,6=     ,7=     ,8=Line ";
	...
	public void Timer(TimerEvent e) {
		...
		int dip = (PortIO.getPort(PortIO.PORTB) & 0xF0) ^ 0xF0;
		if (dip > 0 && dip < 0xF0)
			dip = dip >> 4;
		else {
			dip = (PortIO.getPort(PortIO.PORTB) & 0x0F) ^ 0x0F;
			if (dip == 0x0F)
				dip = 0;
		}
	...
	public void processState(int dipValue) {
		...
		case STATE_FLEE: // 2
			...
			break;
		case STATE_LINE_FOLLOWER: // 4
			currentBehavior = new LineFollowerBehavior(joBot, behaviorServiceTick, 100);
			break;
		case STATE_WALL_FOLLOWER: // 8
			currentBehavior = new WallFollowerBehavior(joBot,
...
//	1=Cal,2=Flee,4=Line,8=Wall

	public void run() {
		System.out.println("UVMdemo Nano NLT Toets V323");
		try {
...
//			The function setPortBDIP in NanoRobot is responsible for this.
	...
	private void showDIP (int dipValue) {
		if (dipValue == 0)
	        System.out.println("1=Cal,2=Flee,4=Line,8=Wall");
