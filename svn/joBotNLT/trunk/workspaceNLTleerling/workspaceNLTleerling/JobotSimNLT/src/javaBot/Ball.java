	...
	public double[] giveSensoryInformationTo(Robot r)
	{
		...
		for (int i = 0; i < r.getSensors().length; i++)
		{
			if (r.getSensors()[i] instanceof IRSensor) {
				...
				retval = irs.checkFOV(r.getLocation(), r.rotation, this.getLocation());
			...
			if (r.getSensors()[i] instanceof AVRSensor) {
				AVRSensor avrs = (AVRSensor) r.getSensors()[i];

				// Check if sensor has this object within it's field of vision
				retval = avrs.checkFOV(r.getLocation(), r.rotation, this.getLocation());

				if (retval > 0)
				{
					// Debug.printDebug("AVR distance " + retval);
					returnValue[i] = retval;
				}
			}
		}
