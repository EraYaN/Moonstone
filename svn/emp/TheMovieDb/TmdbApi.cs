...
namespace TheMovieDb
{
    public class TmdbApi
    {
        ...
        /// <returns></returns>
        public IEnumerable<TmdbMovie> MovieSearch(string title)
        {
            var movies = DownloadData(string.Format(MovieSearchUrl, ApiKey, Escape(title), Language), typeof(TmdbMovies)) as TmdbMovies;
            return movies;
        }
        ...
        {
            var movies = DownloadData(string.Format(MovieGetInfoUrl, ApiKey, id, Language), typeof(TmdbMovies)) as TmdbMovies;
            if (movies != null)
        ...
        private object DownloadData(string url, Type type)
        {
