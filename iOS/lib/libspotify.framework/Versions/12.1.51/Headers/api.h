...
typedef enum sp_error {
  ...
  SP_ERROR_SIGNUP_LOGIN              = 37,
  SP_ERROR_SIGNUP_ACCOUNT_MERGE_FAIL = 38,
...
typedef enum sp_signup_page {
  SP_SIGNUP_PAGE_NONE,
  SP_SIGNUP_PAGE_FACEBOOK_LOGIN,
  SP_SIGNUP_PAGE_ENTER_CREDENTIALS,
  SP_SIGNUP_PAGE_MERGING_ACCOUNTS,
  SP_SIGNUP_PAGE_SPOTIFY_LOGIN,
  SP_SIGNUP_PAGE_POST_TO_OPENGRAPH_OPTION,
  SP_SIGNUP_PAGE_DONE
} sp_signup_page;

typedef enum sp_signup_feature {
  SP_SIGNUP_FEATURE_CONNECT_TO_FACEBOOK = 1 << 0, // Show connect to facebook button
  SP_SIGNUP_FEATURE_MERGE_ACCOUNT       = 1 << 1, // Show merge account option
  SP_SIGNUP_FEATURE_SHOW_LICENSES       = 1 << 2, // Show link to licenses from OG page
  SP_SIGNUP_FEATURE_SHOW_POST_OG_OPTION = 1 << 3, // Show enable posting to Facebook checkbox
} sp_signup_feature;

typedef enum sp_signup_action {
  SP_SIGNUP_ACTION_CONNECT_TO_FACEBOOK,
  SP_SIGNUP_ACTION_CONNECT_TO_FACEBOOK_COMPLETED,
  SP_SIGNUP_ACTION_LOGIN_WITH_SPOTIFY,
  SP_SIGNUP_ACTION_ACCEPT,
  SP_SIGNUP_ACTION_CANCEL_FACEBOOK_CONNECT,
  SP_SIGNUP_ACTION_MERGE_WITH_ACCOUNT,
  SP_SIGNUP_ACTION_GO_BACK,
  SP_SIGNUP_ACTION_CANCEL_SIGNUP,
  SP_SIGNUP_ACTION_ACCEPT_LICENSES
} sp_signup_action;

typedef struct sp_signup_userdata_success {
  bool success;
} sp_signup_userdata_success;

typedef struct sp_signup_userdata_with_scrobble_setting {
  bool should_scrobble;
} sp_signup_userdata_with_scrobble_setting;

typedef struct sp_signup_userdata_with_user_credentials {
  const char *user_name;
  const char *password;
} sp_signup_userdata_with_user_credentials;

typedef struct sp_signup_userdata_with_accepted_licenses {
  const char **licenses;
  int license_count;
} sp_signup_userdata_with_accepted_licenses;
...
typedef struct sp_session_callbacks {
  ...
  void (SP_CALLCONV *show_signup_page)(sp_session *session, sp_signup_page page, bool pageIsLoading, int featureMask, const char *recentUserName);
  void (SP_CALLCONV *show_signup_error_page)(sp_session *session, sp_signup_page page, sp_error error);
  void (SP_CALLCONV *connect_to_facebook)(sp_session *session, const char **permissions, int permission_count);
  void (SP_CALLCONV *show_trial_welcome)(sp_session *session, unsigned int trial_duration);
   /**
 ...
 * @param[in]   user_data The data associated with the action.
 * @return                 One of the following errors, from ::sp_error
 *                         SP_ERROR_OK
 */
SP_LIBEXPORT(sp_error) sp_session_signup_perform_action(sp_session *session, sp_signup_action action, void *user_data);
 ...
 *
 * @param[in]    session    Your session object.
 * @param[out]   licenses       Pointer to an array of char * to fill with licenses. May be NULL.
 * @param[in]    num_licenses   The size of your licenses array.
 * @return       The number of licenses available.
 */
SP_LIBEXPORT(int) sp_session_signup_get_unaccepted_licenses(sp_session *session, const char **licenses, int num_licenses);
