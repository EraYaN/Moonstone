using System;
using System.Collections.Generic;
using System.Configuration;
using System.Data;
using System.IO;
using System.Linq;
using System.Threading.Tasks;
using System.Windows;

namespace SpotiFire.TestWPF
{
    /// <summary>
    /// Interaction logic for App.xaml
    /// </summary>
    public partial class App : Application
    {
        const string CLIENT_NAME = "TestAppWPF";
        static byte[] key = new byte[] { , };
        static string cache = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), CLIENT_NAME, "cache");
        static string settings = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), CLIENT_NAME, "settings");
        static string userAgent = CLIENT_NAME;

        protected override void OnStartup(StartupEventArgs e)
        {
            Spotify.CreateSession(key, cache, settings, userAgent);
            base.OnStartup(e);
        }
    }
}
